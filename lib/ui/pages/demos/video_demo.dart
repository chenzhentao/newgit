...
class SingleVideoCard extends StatefulWidget {
  const SingleVideoCard({Key key, this.controller, this.firstScreen})
      : super(key: key);
...
class SingleVideoState extends State<SingleVideoCard> {
  double width = 0;
  double height = 0;
  ...
  Future getRect() async {
    if (widget.firstScreen != null) {
      Image image =
          new Image(image: new CachedNetworkImageProvider(widget.firstScreen));
  ...
  Widget _buildInlineVideo() {
    return new ConstrainedBox(
      ...
      child: AspectRatio(
        aspectRatio: (width != 0 && height != 0) ? width / height : 3 / 2,
        child: Hero(
          tag: widget.controller,
          child: VideoPlayPause(widget.controller,
  ...
  Widget build(BuildContext context) {
    ...
    void pushFullScreenWidget() {
      final TransitionRoute<void> route = PageRouteBuilder<void>(
        settings: RouteSettings(name: "", isInitialRoute: false),
...
class VideoPlayPause extends StatefulWidget {
  const VideoPlayPause(this.controller, {this.firstScreen});
...
class _VideoPlayPauseState extends State<VideoPlayPause> {
  ...
  String get _firstScreen => widget.firstScreen;
  ...
  bool firstScreenOffStage = false;
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: <Widget>[
        GestureDetector(
          child: Stack(
              ...
              children: <Widget>[
                ...
                VideoPlayerLoading(controller),
                new Offstage(
                  offstage: firstScreenOffStage,
                  child: new CachedNetworkImage(imageUrl: widget.firstScreen),
                ),
              ]),
          onTap: () {
        ...
        new Offstage(
          offstage: !firstScreenOffStage,
          child: Center(child: imageFadeAnimation),
        ),
      ],
    );
  }
